...
dataset_path = {  'cmt': os.path.join(base_dir, ''),
                   }
dataset_path = {  'cmt': os.path.join(base_dir, ''),
                   }
...
class Dataset(object):
    ...
    """
    def __init__(self, name='cmt', max_len=max_len, base_dir=base_dir, label_dict=cfg.reverse_label_dict): # label_dict  label_dict_with_rects 5434+1
...
class CMT(Dataset):
    def __init__(self, name='cmt'):
        super(CMT, self).__init__(name=name)

    def load_data(self):
        # label_folder = os.path.join(self.base_dir, '')  # gt_unicode gt
        label_folder = base_dir
        img_folder = base_dir
        with open ('dataclean_and_agm.txt', 'r', encoding='utf-8') as f:
            lines = f.readlines()
            t = -1
            print(len(lines))
            for line in lines:
                ...
                # print(img_name)
                            #   '/home/quannd/Vin-BigData/Attention_OCR_Batch/dataset/imageout/0.jpg'
                # img_name =  '/home/quannd/Vin-BigData/Attention_OCR_Batch/dataset/imageout/0.jpg'
...
if __name__=='__main__':
    ...
    CMT =CMT()
    CMT.load_data()
    print(len(CMT.filenames))
    filenames = CMT.filenames
    labels = CMT.labels
    masks = CMT.masks
    bboxes = CMT.bboxes
    points = CMT.points
    ...
    print(len(filenames))
    for i in range(0,6):
        dataset = {"filenames":filenames[i*500000:(i+1)*500000], "labels":labels[i*500000:(i+1)*500000], "masks":masks[i*500000:(i+1)*500000], "bboxes":bboxes[i*500000:(i+1)*500000], "points":points[i*500000:(i+1)*500000]}
        np.save('data_10_4_2020_dataclean_and_agm_'+str(i)+'.npy', dataset)
