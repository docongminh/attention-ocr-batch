...
def largest_size_at_most(height, width, largest_side, max_scale,largest_side_width):
    """
    Compute resized image size with limited max scale. 
    """

    # scale = largest_side/height if height/largest_side>width/largest_side_width else largest_side_width/width
    # scale = min(scale, max_scale)
    new_height = largest_side 
    new_width = int(width*largest_side/height)
    new_width = min(new_width,largest_side_width)

    # new_height, new_width = height * scale, width * scale
    return new_height, new_width
import random
def aspect_preserving_resize(image, largest_side,largest_side_width, max_scale=4.):
    """
    Resize image with perserved aspect and limited max scale.
    """
    height, width = image.shape[:2]
    new_height, new_width = largest_size_at_most(height, width, largest_side, max_scale,largest_side_width)
    new_height = max(new_height, cfg.stride)
    new_width = max(new_width, cfg.stride)
    r = random.randint(0,3)
    a=[cv2.INTER_LINEAR,cv2.INTER_NEAREST,cv2.INTER_CUBIC,cv2.INTER_AREA]
    resized_image = cv2.resize(image, (int(new_width), int(new_height)),a[r])

    return resized_image

def padding_image(image, padding_size,padding_size_width):
    """
    Padding arbitrary-shaped text image to square for tensorflow batch training.
    """
    height, width = image.shape[:2]
    padding_h = padding_size - height
    padding_w = padding_size_width - width
    ...
    padding_img = cv2.copyMakeBorder(image, padding_top, padding_down, padding_left, padding_right, borderType=cv2.BORDER_CONSTANT, value=[255,255,255])
...
class TextDataPreprocessor:
    ...
    def __call__(self, roidb):
        ...
        largest_side_width = np.random.randint(cfg.crop_min_size_width, cfg.image_size_width)
        img = aspect_preserving_resize(img, largest_side,largest_side_width)
        
        img, crop_bbox = padding_image(img, cfg.image_size,cfg.image_size_width)

        # normalized_bbox = [coord/cfg.image_size for coord in crop_bbox]
# (padding_top, padding_left, height, width)
        normalized_bbox = [crop_box[0]/cfg.image_size,crop_box[1]/cfg.image_size_width,crop_box[2]/cfg.image_size,crop_box[3]/cfg.image_size_width]
...
def get_roidb(dataset_name):
    ...
    # np.load = np_load_old
    roidb = []
    for dataset_name_x in dataset_name:
        dataset = np.load(dataset_name_x)[()]
        filenames, labels, masks, bboxes, points = dataset["filenames"], dataset["labels"], dataset["masks"], dataset["bboxes"], dataset["points"]
        filenames, labels, masks, bboxes, points = shuffle(np.array(filenames),np.array(labels),np.array(masks),np.array(bboxes),np.array(points))
        # ls = list(zip(filenames, labels, masks, bboxes, points))
        # random.shuffle(ls)
        # print(len((*ls)))
...
def get_batch_train_dataflow(roidbs, batch_size):
    ...
    def preprocess(roidb_batch):
        ...
        for roidb in roidb_batch:
            ...
            # normalized_bbox = [coord/cfg.image_size for coord in crop_bbox]
    # (padding_top, padding_left, height, width)
            normalized_bbox = [crop_bbox[0]/cfg.image_size,crop_bbox[1]/cfg.image_size_width,crop_bbox[2]/cfg.image_size,crop_bbox[3]/cfg.image_size_width]
            img = img.astype("float32")/255.
